...
(defun select-parents (parents domain stream)
  (let ((parents-list (gensym "PARENTS-"))
	(domain-list (gensym "DOMAIN-"))
	(before-button (gensym "BEFORE-"))
	(after-button (gensym "AFTER-"))
	(add-button (gensym "ADD-"))
	(remove-button (gensym "REMOVE-")))
    (with-html-output (stream)
      (:table
       (:tbody
	(:tr
	 (:td :rowspan "4"
	      (:select :id parents-list
		       :multiple "multiple"
	 ...
	 (:td :rowspan "4"
	      (:select :id domain-list
		       :multiple "multiple"
		       :style "height:100%"
		       (loop for item in domain
			  do
			    (htm
			     (:option :value (cfg::name item)
				      (str (cfg::title item))))))))
	(:tr
	 (:td
	...
	(:tr
	 (:td
	  (:input :id before-button
		  :type "button"
		  :value "Before")))
	(:tr
	 (:td
	  (:input :id after-button
		  :type "button"
		  :value "After")))))
      (:script :language "javascript"
	       (flet ((jquery-id (object)
			(format nil "#~A" object)))
		 (str
		  (ps*
		   `(chain ($ document)
			   (ready (lambda ()
				    ;; Add button
				    (chain ($ ,(jquery-id add-button))
					   (click (lambda ()
						    (if (chain ($ ,(jquery-id domain-list))
							       (val))
							(progn
							  (chain ($ ,(jquery-id parents-list))
								 (append
								  (concatenate 'string
									       "<option value="
									       (chain ($ ,(jquery-id domain-list))
										      (val))
									       " selected='selected'>"
									       (chain
										($ (concatenate 'string ,(jquery-id domain-list)
												" option[value='"
												(chain ($ ,(jquery-id domain-list))
												       (val))
												"']"))
							  ...
							  (chain ($ (concatenate 'string ,(jquery-id domain-list)
										 " option[value='"
										 (chain ($ ,(jquery-id domain-list))
											(val))
										 "']"))
				    ...
				    ;; Remove button
				    (chain ($ ,(jquery-id remove-button))
					   (click (lambda ()
						    (if (chain ($ ,(jquery-id parents-list))
							       (val))
							(progn
							  (chain ($ ,(jquery-id domain-list))
								 (append
								  (concatenate 'string
									       ...
									       " selected='selected'>"
									       (chain
										($ (concatenate 'string ,(jquery-id parents-list)
												" option[value='"
												(chain ($ ,(jquery-id parents-list))
												       (val))
												"']"))
										(text))
									       "</option>")))
							  (chain ($ (concatenate 'string ,(jquery-id parents-list)
										 " option[value='"
										 (chain ($ ,(jquery-id parents-list))
											(val))
										 "']"))
